...
def draw_menu():
    ...
    play_button = pygame.Rect((302, 330), (245, 75))  # Fixed typo in pygame.Rect
...
def draw_gun():
    global shot  # Added global declaration
    mouse_pos = pygame.mouse.get_pos()
    ...
    if mouse_pos[0] < WIDTH / 2:
        ...
        if mouse_pos[1] < 600:
            ...
            if clicks[0]:
                ...
                shot = True  # Set shot to True on mouse click
    else:
        gun = guns
        if mouse_pos[1] < 600:
...
def draw_level(coords):
    ...
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            target_rects[i].append(pygame.Rect((coords[i][j][0] + 20, coords[i][j][0]),
                                               (60 - i * 12, 60 - i * 12)))
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if (0 < mouse_position[0] < WIDTH) and (0 < mouse_position[1] < HEIGHT - 200):  # Fixed typo
                shot = True
